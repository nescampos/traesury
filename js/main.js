...
async function getContract() {
  const contract = await aeSdk.initializeContract({ sourceCode : investmentContractCode });
  var aci = contract._aci;
  contractPublic = await aeSdk.initializeContract({ aci, address : investmentContractAddress });
}
...
async function getProposalById(){
  ...
  if(contractPublic != undefined) {
    ...
    if(clubs.decodedResult != undefined) {
      ...
      if(clubs.decodedResult.status == 'Pending' && clubs.decodedResult.creator == aeWalletAddress) {
...
async function createProposal() {
  if(contractPublic != null) {
    ...
    if(privateKey !== '')
    {
      ...
      var proposalId = await contractPublic.createProposal(clubId,proposal_amount, proposal_address, proposal_description);
      ...
      $('#successCreateProposal').text("Proposal created successfully with id: " + proposalId.decodedResult);
    } else {
      $('.valid-feedback').css('display','none');
      $('.loading_message_creating').css("display","none");
        $('.invalid-feedback').css('display','block');
...
async function createClub() {
  if(contractPublic != null) {
    ...
    if(privateKey !== '')
    {
      ...
      $('#successCreateClub').text("Club created successfully with id: " + clubId.decodedResult);
    } else {
      $('.valid-feedback').css('display','none');
        $('.invalid-feedback').css('display','block');
        $('.loading_message_creating').css("display","none");
  ...
  if(privateKey !== '')
  {
    ...
    if(clubId != null) {
      ...
      if(contractPublic != undefined) {
        ...
        try {
          await contractPublic.contributeToClub(clubId,{amount: amountAE, gasLimit: 1000000});
        } catch(e) {
          $('.successContributeClub').css('display','none');
          $('.errorContributeClub').css("display","block");
  ...
  if(contractPublic != undefined) {
    ...
    clubs.decodedResult.forEach((valor, clave) => 
      { valor.members.forEach((valor1, clave1) => 
        { valor1.address == localStorage.getItem('aeWalletAddress')? myclubs.push(valor) : ""}) })
    if(myclubs.length > 0) {
